...
2. the user selects their changes
3. the old version is 'deleted' i.e. its delete date is updated. 
4. a new version with the same pay_id, create_date, and user_id is created, with any changed values represented there
5. a new uuid (id) is generated for the new version, and its update_date is set to the current timestamp (we can use this to see when changes are made without updating create_date).
6. the new version is pushed onto the stack as the sole, non-deleted version of the update. 
...
- Multiple Users: As Contacts/Pays are already set up to be per-user, implementing multiple users/login would be the next step. 
